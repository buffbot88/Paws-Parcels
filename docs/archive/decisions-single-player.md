...
>
> This is the **original single-player locked decisions file**, preserved for history.
> It was superseded on **2026-08-03 (Phase 3.5 documentation pivot)** by the current
> [`design/decisions.md`](../../design/decisions.md), which locks the **online
> server-authoritative multiplayer** decisions on top of the retained client ones.
>
> Decisions here that are still valid (resolution, tile size, zone IDs, ID
> conventions, Stamps economy) carry over to the new file; everything about
> localStorage progression is **replaced by MySQL server persistence**.
> Status marker: 🗃 archived

# Paws & Parcels — Locked Design Decisions (ARCHIVED — single-player)
