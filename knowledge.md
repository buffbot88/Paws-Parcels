...
- Dev server: `npm run dev:server` — SQLite at `server/data/paws-and-parcels.sqlite` (auto-created + migrated on boot), health check `http://localhost:3001/api/health`, WebSocket on `/ws`
- Build: `npm run build` (outputs `dist/`; `base: './'` for static hosting)
- Typecheck: `npm run typecheck`
- Test: `npm test` (Vitest) — 248 unit tests in `tests/`
