...
- `design/decisions.md` — locked decisions (2.5D isometric, server-authoritative, class system, MySQL persistence, safe hub)
- `design/architecture.md` — client/server/MySQL responsibilities, HTTP + WebSocket, auth flow, reconnection, tick model
- `design/database-schema.md` — MySQL entities (accounts, characters, inventory, quests, dungeons, audit), PK/FK, ownership, auth handling
...
- `design/world-map.md` — tile-unit sketches for the 2 MVP zones (post office + Bramble Patch)
...
- **Database:** MySQL — accessed only by the server. Passwords hashed (argon2id).
...
- **Persistence:** MySQL for all gameplay state. localStorage limited to client settings (audio, UI, controls).
- **Maps:** custom JSON (ASCII `rows` + spawn/transitions); **assets:** Aseprite → sprite sheets + JSON atlases
- **Data:** JSON content files under `src/data/` (npcs, items, quests, upgrades, dialogue) — may be mirrored to MySQL seed data
- **Deployment:** static hosting (Vercel/Netlify) for client; containerized server + managed MySQL
...
- The old `SaveData` / `localStorage` progression model is superseded by MySQL persistence.