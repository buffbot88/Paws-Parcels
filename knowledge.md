...
**Status: Phase 3 (Interaction & Dialogue) complete.** Game boots (Vite 8 + Phaser 4), both zones walkable with collision + camera + transitions, all 5 NPCs + map objects interactable via a DOM dialogue panel. Architecture below is from `BuildPlan.md` (the authoritative build doc), with the deviations noted in `design/decisions.md` + knowledge.md (Phaser 4 over Phaser 3; custom-JSON maps over Tiled).
...
- **Player/input (Phase 2-3):** `src/entities/Player.ts` (arcade sprite, 180 px/s, small 28×20 body, facing) driven by `src/systems/InputSystem.ts` (keyboard WASD/arrows + pointer-drag joystick — Phaser 4 has no built-in joystick; Phase 3 added tap-vs-drag disambiguation: joystick only engages past 8px, so taps fire `consumeTap()`, and E/Space queue `consumeInteract()`)
- **NPCs/interaction (Phase 3):** `src/entities/NPC.ts` (tinted blob + name tag, static physics body), `src/systems/InteractionSystem.ts` (`getFocused` nearest-in-range, NPCs win ties), `src/systems/DialogueService.ts` (`selectDialogueSet(sets, npcId, level)` — friendship-range aware, falls back to lowest gate), `src/ui/DialoguePanel.ts` (DOM overlay: click/tap/Enter/E/Space advance, Esc closes, focus in/out). Map JSONs gained `interactables[]` (counter/quest-board/mailbox/shop/sign with flavor lines). Dialogue is opened from `OverworldScene.startInteraction`; `selectDialogueSet` is called with level 0 until Phase 6 adds friendship state
- **DOM layout:** `index.html` (`#game-container` > `#ui-overlay`) + `src/styles/{global,game-ui}.css` center a 16:9 container; Phaser canvas fills it; UI overlays live in the DOM
...
**Currently in place:** `src/data/*.json` (content), `src/types/*.ts` (typed models matching the JSON: `PlayerState`, `SaveData`, `ItemDefinition`, `QuestDefinition`, `NPC`, plus `UpgradeDefinition`/`DialogueSet`/`ContentData`), `src/systems/ContentValidator.ts` + `src/systems/MapValidator.ts` (shared validation rules incl. interactables + NPC home-tile walkability), `src/systems/{DialogueService,InteractionSystem}.ts` (Phase 3 logic), `src/ui/DialoguePanel.ts` — **69 Vitest tests total** (content, map, systems). `scripts/validate-content.ts` wraps the same validators — single source of truth.
